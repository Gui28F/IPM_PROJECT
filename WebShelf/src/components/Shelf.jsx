...
export default function MonthlySuggestions() {
  ...
  const currentShelfIndex = shelves.findIndex(shelf => shelf.id === parseInt(id));
  console.log(currentShelfIndex);
  const currentShelf = shelves[currentShelfIndex];

  const shelfBooks = books.filter(book => currentShelf.booksIDs.includes(book.id));
          ...
          >
            ...
            >
              ...
              <Link to={"/books/" + book.id}>
                <img src={book.imageUrl} alt={book.title} className={clsx("h-full w-48 bg-cover", animationStyle)} />
              </Link>
            </div>
          </button>
        ))}
